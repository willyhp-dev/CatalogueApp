...
public class ProductModel extends RecordModel {
        ...
        @Null(message = "Product id must be null", groups = {
                        PostingNew.class })
        @NotNull(message = "Product Id cannot be null", groups = {
                        UpdatingById.class, DeletingById.class })
        @PositiveOrZero(message = "Product Id must not be less than 0", groups = {
                        GettingAllByCriteria.class, RequestMethodById.class })
        private Integer id;

        @NotBlank(message = "Product Name cannot be blank", groups = {
                        PostingNew.class })
        @Pattern(regexp = "^[a-zA-Z]+(?:[\\s.]+[a-zA-Z]+)*$", message = "Product Name pattern must be words only", groups = {
                        PostingNew.class, GettingAllByCriteria.class,
                        UpdatingById.class })
        private String name;

        @NotNull(message = "Product Quantity cannot be null", groups = {
                        PostingNew.class })
        // @Pattern(regexp = "^[0-9]$", message = "Product Quantity me be
        // numeric",
