...
public class Usermodel {
    ...
    @PositiveOrZero(message = "User id must be not be less than 0", groups = { GettingAllByCriteria.class })
    private Integer id;
    @NotBlank(message = "User fullname cannot be blank", groups = { PostingNew.class })
    @Pattern(regexp = "^[a-zA-Z]+(?:[\\s.]+[a-zA-Z]+)*$", message = "User Fullname pattern must be words only", 
     groups = {
    ...
    @Pattern(regexp = "^[0,+62][0-9]{9,12}$", message = "User Call Number partern input is invalid ,Must be numeric, and Start with 0 , +62"
    ,groups = {PostingNew.class, GettingAllByCriteria.class,UpdatingById.class})
    private String callnumber;

    @PositiveOrZero(message = "User must not be less than 0", groups = { PostingNew.class,
